...
class ColumnSchema
{
    ...
    public static function image(): array
    {
        return [
            FileUpload::make("image")
                ...
                ->label("Afbeelding")
                ->imageEditor()
                ->imageEditorAspectRatios(["134:125"])
                ->preserveFilenames()
    ...
    public static function contact(): array
    {
        ...
        $socials = collect($settings->accounts)
            ->filter(fn($value) => !empty($value["url"]))
        ...
        return [
            ...
            CheckboxList::make("social")
                ...
                ->helperText(
                    'Deze accounts worden getoond onder het "Volg ons" gedeelte'
                )
    ...
    public static function bank(): array
    {
        ...
        $accounts = collect($settings->accounts)
            ->filter(fn($value) => empty($value["url"]))
            ->mapWithKeys(fn($value) => [$value["key"] => $value["label"]]);
        ...
        return [
            CheckboxList::make("accounts")
                ->label("Bank gegevens")
                ->helperText(
                    "Deze accounts worden getoond in het financiële gegevens blok"
                )
    ...
    public static function resolveViewAttributes(array $attributes): array
    {
        ...
        if (isset($attributes["contact"]) && !empty($attributes["contact"])) {
            $attributes["contact"] = self::resolveContact(
                $attributes["contact"]
            );
        }

        if (isset($attributes["social"]) && !empty($attributes["social"])) {
            $attributes["social"] = self::resolveAccounts(
                $attributes["social"]
            );
        }

        if (isset($attributes["accounts"]) && !empty($attributes["accounts"])) {
            $attributes["accounts"] = self::resolveAccounts(
                $attributes["accounts"]
            );
        }
    ...
    public static function resolveAccounts(array $values): array
    {
        return array_map(
            fn(
                string $value
            ): Account => self::getContactSettings()->getAccount($value),
            $values
        );
    }
    ...
    protected static function getContactSettings(): ContactInformationSettings
    {
        if (!isset(self::$contactSettings)) {
            self::$contactSettings = app()->make(
                ContactInformationSettings::class
            );
        }
